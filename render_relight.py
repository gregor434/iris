...
def main():
    ...
    parser.add_argument(
        "--mode", type=str, default="train_val", choices=["train_val", "traj"]
    )
    ...
    scene_dict = load_scene_dict(
        ...
        fov=fov,
        img_hw=(h, w),
        max_depth=args.indir_depth + 2,
        mesh_path=mesh_path,
        mesh_type=mesh_type,
        emitter_path=emitter_path,
        brdf_path=str(last_ckpt),
    )
    disco_cfg = (
        copy.deepcopy(scene_dict["disco_ball"]) if "disco_ball" in scene_dict else None
    )
    ...
    for i in tqdm(range(len(c2w_all))):
        ...
        params["PerspectiveCamera.to_world"] = mitsuba.Transform4f.look_at(
            origin=t, target=t + forward, up=up
        )
        params.update()
