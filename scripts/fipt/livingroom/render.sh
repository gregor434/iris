...
SCENE='livingroom'
LDR_IMG_DIR='Image'
EXP='fipt_syn_livingroom'
VAL_FRAME=10 # bathroom: 10, bedroom: 10, livingroom:10, classroom:64, conferenceroom: 4
CRF_BASIS=3
...
if [[ "${RUN_RELIGHT}" == "1" ]]; then
  python render_relight.py --experiment_name $EXP --device 0\
          ...
          --split $RELIGHT_SPLIT\
          --ldr_img_dir $LDR_IMG_DIR \
          --light_cfg 'configs/fipt/livingroom/relight_0.yaml' \
          --SPP $SPP --spp $spp --crf_basis $CRF_BASIS
fi
