...
python initialize.py --experiment_name $EXP --max_epochs 3 \
  ...
  --has_part $HAS_PART --ldr_img_dir $LDR_IMG_DIR --val_frame $VAL_FRAME --SPP 128 --spp 32 --crf_basis $CRF_BASIS
...
python train_brdf_crf.py --experiment_name $EXP \
  ...
  --dataset $DATASET $DATASET_ROOT$SCENE \
  --has_part $HAS_PART --ldr_img_dir $LDR_IMG_DIR --val_frame $VAL_FRAME --SPP 128 --spp 32 --lp 0.005 --la 0.01 --l_crf_weight 0.01 --crf_basis $CRF_BASIS
