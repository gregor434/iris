...
def main():
    ...
    # create folders
    dir_out = {}
    for name in ['rgb', 'kd', 'albedo', 'a_prime', 'roughness', 'metallic', 'emission', 'slf', 'merge']:
    ...
    for i in tqdm(range(len(dataset))):
        ...
        with torch.no_grad():
            for b0 in range(0, len(rays_x), chunk_size):
                ...
                for _ in range(SPP//spp):
                    ...
                    slf[b0:b1] += slf_.reshape(-1, spp, 3).mean(1)
        ...
        merge = np.concatenate(
            [L_gt, L_ldr, pred_kd, pred_albedo, pred_a_prime, roughness, metallic, emission],
            axis=1,
        )
