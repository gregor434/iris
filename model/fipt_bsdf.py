...
class FIPTBSDF(mitsuba.BSDF):
    def __init__(self, props):
        ...
        mask = torch.load(
            os.path.join(props["emitter_path"], "vslf.npz"), map_location="cpu"
        )
        ...
        self.is_emitter = torch.load(
            os.path.join(props["emitter_path"], "emitter.pth")
        )["is_emitter"].to(device)

        # specify flags
        reflection_flags = (
            mitsuba.BSDFFlags.SpatiallyVarying
            | mitsuba.BSDFFlags.DiffuseReflection
            | mitsuba.BSDFFlags.FrontSide
            | mitsuba.BSDFFlags.BackSide
        )
    ...
    def sample(self, ctx, si, sample1, sample2, active):
        ...
        wo, pdf, brdf_weight = self.material_net.sample_brdf(
            sample1.torch().reshape(-1), sample2.torch(), wi, normal, mat
        )
        brdf_weight[self.is_emitter[triangle_idx]] = (
            1.0  # increase from 0 to 1 to fill the emitter region color
        )

        pdf_mi = mitsuba.Float(pdf.squeeze(-1))
        wo_mi = mitsuba.Vector3f(wo[..., 0], wo[..., 1], wo[..., 2])
        wo_mi = si.to_local(wo_mi)
        value_mi = mitsuba.Vector3f(
            brdf_weight[..., 0], brdf_weight[..., 1], brdf_weight[..., 2]
        )
