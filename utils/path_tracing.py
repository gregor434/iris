...
def path_tracing_det_diff(
    ...
    # next intersection
    position_next, normal, _, triangle_idx, vis = ray_intersect(
        scene, position + mitsuba.math.RayEpsilon * wi, wi
    )

    # get surface BRDF and Le
    mat_next = material_net(position_next)
    Le, emit_pdf, valid_next = emitter_net.eval_emitter(
        position_next, wi, triangle_idx, mat_next["roughness"]
    )
    ...
    with torch.no_grad():
        L_indir = trace_indirect(
            scene, emitter_net, material_net, position, wo, normal, indir_depth
        )
...
def path_tracing_det_spec(
    ...
    # get surface BRDF and Le
    mat_next = material_net(position_next)
    Le, _, valid_next = emitter_net.eval_emitter(
        position_next, wi, triangle_idx, mat_next["roughness"]
    )
...
def path_tracing(
    scene, emitter_net, material_net, rays_o, rays_d, dx_du, dy_dv, spp, indir_depth
):
    ...
    wi = NF.normalize(
        rays_d[:, None] + dx_du[:, None] * du + dy_dv[:, None] * dv, dim=-1
    ).reshape(-1, 3)
    ...
    # visibility test
    emit_position, emit_normal, _, triangle_idx, emit_valid = ray_intersect(
        scene, position + mitsuba.math.RayEpsilon * wi, wi
    )
    ...
    G = (-wi * emit_normal).sum(-1).abs() / (emit_position - position).pow(2).sum(
        -1
    ).clamp_min(1e-6)
    ...
    G = (-normal * wi).sum(-1).abs() / (position - position_next).pow(2).sum(
        -1
    ).clamp_min(1e-6)
    ...
    w_mis = torch.where(
        (brdf_pdf > 0) & (~emit_pdf.isinf()),
        brdf_pdf * brdf_pdf / (emit_pdf * emit_pdf + brdf_pdf * brdf_pdf),
        0,
    )
...
def path_tracing_single(
    scene, emitter_net, material_net, rays_o, rays_d, dx_du, dy_dv, spp
):
    """Path trace current scene
    Args:
        scene: mitsuba scene
    ...
    w_mis = torch.where(
        ...
        emit_pdf
        * emit_pdf
        / (emit_pdf * emit_pdf + brdf_pdf * brdf_pdf).clamp_min(1e-6),
        0,
    )
    w_mis[emit_pdf.isinf() | (brdf_pdf == 0)] = 1
    L[active_next] += emit_brdf * emit_weight * w_mis
    ...
    # evaluate Le
    Le, emit_pdf, valid_next = emitter_net.eval_emitter(
        position_next, wi, triangle_idx, mat_next["roughness"], trace_roughness
    )
    G = (-normal * wi).sum(-1).abs() / (position - position_next).pow(2).sum(
        -1
    ).clamp_min(1e-6)
...
def trace_indirect(scene, emitter_net, material_net, position, wo, normal, indir_depth):
    ...
    for depth in range(indir_depth):
        ...
        emit_vis = (~emit_valid) | (
            emit_triangle_idx == triangle_idx
        )  # visible = not env (valid) + same triangle id
        emit_weight, _, _ = emitter_net.eval_emitter(emit_position, wi, triangle_idx)
