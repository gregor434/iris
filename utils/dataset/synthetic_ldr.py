...
class SyntheticSplitLayout:
    ...
    def __init__(self, scene_root, split_root, img_dir=None):
        ...
        requested_input = os.path.join(split_root, img_dir) if img_dir is not None else None
        ...
        self.aov_dirs = {
            "kd": _first_existing(os.path.join(split_root, "aovs", "kd"), os.path.join(split_root, "kd"), os.path.join(split_root, "DiffCol")),
            "albedo": _first_existing(os.path.join(split_root, "aovs", "albedo"), os.path.join(split_root, "albedo_pure")),
            "a_prime": _first_existing(os.path.join(split_root, "aovs", "a_prime"), os.path.join(split_root, "a_prime"), os.path.join(split_root, "albedo")),
            "roughness": _first_existing(os.path.join(split_root, "aovs", "roughness"), os.path.join(split_root, "roughness"), os.path.join(split_root, "Roughness")),
            "emission": _first_existing(os.path.join(split_root, "aovs", "emission"), os.path.join(split_root, "emission"), os.path.join(split_root, "Emit")),
        }
        self.label_dirs = {
            "material_id": _first_existing(os.path.join(split_root, "labels", "material_id"), os.path.join(split_root, "material_id"), os.path.join(split_root, "IndexMA")),
            "segmentation": _first_existing(os.path.join(split_root, "labels", "segmentation"), os.path.join(split_root, "segmentation")),
        }
        self.prior_dirs = {
    ...
    def label_dir(self, name, required=True):
        ...
        if path is None and required:
            raise FileNotFoundError(f"Missing required synthetic label directory: {name}")
        return path

    def prior_dir(self, name, required=True):
        path = self.prior_dirs.get(name)
        if path is None and required:
            raise FileNotFoundError(f"Missing required synthetic prior directory: {name}")
        return path
    ...
    def hdr_probe_path(self):
        return _frame_path(self.hdr_dir, 0, "{frame:03d}_0001.exr", "{frame:03d}_0001.png")
    ...
    def aov_path(self, name, frame_index, required=True):
        ...
        if name in ("albedo", "a_prime"):
            return _frame_path(directory, frame_index, "{frame:03d}.exr", "{frame:03d}_0001.exr", required=required)
        return _frame_path(directory, frame_index, "{frame:03d}_0001.exr", required=required)

    def label_path(self, name, frame_index, required=True):
        directory = self.label_dir(name, required=required)
        if directory is None:
            return None
        patterns = ("{frame:03d}.exr", "{frame:03d}_0001.exr") if name == "segmentation" else ("{frame:03d}_0001.exr",)
        return _frame_path(directory, frame_index, *patterns, required=required)

    def albedo_prior_path(self, frame_index):
        return _frame_path(self.prior_dir("albedo"), frame_index, "{frame:03d}_0001.png", "{frame:03d}.png")
...
class SyntheticDatasetLDR(Dataset):
    ...
    def __init__(self, 
        ...
        self.pixel=pixel
        self.split = split
        self.has_segmentation = self.layout.label_dir('segmentation', required=False) is not None
        ...
        train_layout = SyntheticSplitLayout(root_dir, os.path.join(root_dir, 'train'), img_dir=img_dir)
        ...
        if self.pixel:
            ...
            for cur_idx in range(len(self.meta['frames'])):
                ...
                kd = open_exr(self.layout.aov_path('kd', cur_idx),self.img_hw).reshape(-1,3)
                a_prime_gt = _load_optional_exr(self.layout.aov_path('a_prime', cur_idx, required=False), self.img_hw)
                a_prime = a_prime_gt.reshape(-1,3) if a_prime_gt is not None else torch.zeros_like(kd)
                albedo_gt = _load_optional_exr(self.layout.aov_path('albedo', cur_idx, required=False), self.img_hw)
                albedo = albedo_gt.reshape(-1,3) if albedo_gt is not None else torch.zeros_like(a_prime)
                roughness = open_exr(self.layout.aov_path('roughness', cur_idx),self.img_hw).reshape(-1,3)[...,:1]
                emission = open_exr(self.layout.aov_path('emission', cur_idx),self.img_hw).reshape(-1,3)
                ...
                if self.ray_diff==False:
                    ...
                    ray_tensors = [rays_o, rays_d, kd, albedo, a_prime, roughness, emission, material_id]
                    ...
                    ray_tensors = [rays_o, rays_d, dxdu, dydv, kd, albedo, a_prime, roughness, emission, material_id]
                    if segmentation is not None:
                        ray_tensors.append(segmentation)
                    self.all_rays += [torch.cat(ray_tensors, 1)]
    ...
    def __getitem__(self, idx):
        ...
        if self.pixel: 
            ...
            albedo_gt = _load_optional_exr(self.layout.aov_path('albedo', cur_idx, required=False), self.img_hw)
            albedo = albedo_gt.reshape(-1,3) if albedo_gt is not None else torch.zeros_like(a_prime)
            roughness = open_exr(self.layout.aov_path('roughness', cur_idx),self.img_hw).reshape(-1,3)[...,0]
            emission = open_exr(self.layout.aov_path('emission', cur_idx),self.img_hw).reshape(-1,3)
            material_id = open_exr_mask(
                self.layout.label_path('material_id', cur_idx),
                self.img_hw,
